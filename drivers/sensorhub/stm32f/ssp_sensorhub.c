...
void ssp_sensorhub_report_notice(struct ssp_data *ssp_data, char notice)
{
    ...
    if (notice == MSG2SSP_AP_STATUS_RESET) {
        if ((ssp_data->callstack_data == NULL) && (ssp_data->is_reset_from_sysfs == false) && (ssp_data->is_reset_from_kernel == false)) {
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_1,
                             CALLSTACK_RECEIVE_NONE);
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_2,
                             CALLSTACK_RECEIVE_NONE);
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_3,
                             CALLSTACK_RECEIVE_NONE);
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_4,
                             CALLSTACK_RECEIVE_NONE);
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_5,
                             CALLSTACK_RECEIVE_NONE);
        }
        else if(ssp_data->callstack_data != NULL){
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_1,
                             *((uint32_t *)&ssp_data->callstack_data[0]));
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_2,
                             *((uint32_t *)&ssp_data->callstack_data[4]));
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_3,
                             *((uint32_t *)&ssp_data->callstack_data[8]));
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_4,
                             *((uint32_t *)&ssp_data->callstack_data[12]));
            input_report_rel(hub_data->sensorhub_input_dev, CALLSTACK_5,
                             *((uint32_t *)&ssp_data->callstack_data[16]));

            kfree(ssp_data->callstack_data);
            ssp_data->callstack_data = NULL;
        }
...
int ssp_sensorhub_initialize(struct ssp_data *ssp_data)
{
    ...
    input_set_capability(hub_data->sensorhub_input_dev, EV_REL, CALLSTACK_1);
    input_set_capability(hub_data->sensorhub_input_dev, EV_REL, CALLSTACK_2);
    input_set_capability(hub_data->sensorhub_input_dev, EV_REL, CALLSTACK_3);
    input_set_capability(hub_data->sensorhub_input_dev, EV_REL, CALLSTACK_4);
    input_set_capability(hub_data->sensorhub_input_dev, EV_REL, CALLSTACK_5);
