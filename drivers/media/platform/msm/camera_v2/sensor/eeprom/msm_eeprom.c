	...
	void __user *arg)
{
	...
	if (!e_ctrl->cal_data.mapdata)
	{
		pr_err("%s:%d e_ctrl->cal_data.mapdata %p\n", __func__, __LINE__, (void*)e_ctrl->cal_data.mapdata);
		return -EFAULT;
	}
	...
	rc = copy_to_user(ptr_dest, e_ctrl->cal_data.mapdata + cdata.cfg.read_data.addr,
		cdata.cfg.read_data.num_bytes);
	pr_err("%s:%d e_ctrl->cal_data.mapdata %p\n", __func__, __LINE__, (void*)e_ctrl->cal_data.mapdata);
	pr_err("%s:%d cdata.cfg.read_data.addr %u\n", __func__, __LINE__, cdata.cfg.read_data.addr);
	pr_err("cdata.cfg.read_data.num_bytes: %u\n", cdata.cfg.read_data.num_bytes);
	return rc;
}
