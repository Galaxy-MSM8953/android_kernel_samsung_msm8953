...
static void tc300k_gpio_request(struct tc300k_data *data)
{
	...
	if (gpio_is_valid(data->pdata->gpio_en)) {
		ret = gpio_request(data->pdata->gpio_en, "touchkey_en");
		if (ret) {
			dev_err(&data->client->dev, "%s: unable to request touchkey_irq [%d]\n",
					__func__, data->pdata->gpio_int);
		}
		...
		const struct i2c_device_id *id)
{
	...
	data->sec_touchkey = sec_device_create((dev_t)(uintptr_t)&data->sec_touchkey, data, "sec_touchkey");
